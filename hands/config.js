...
export const config = {
  ...
  screening: {
    ...
    maxFeeActiveTvlRatio: u.maxFeeActiveTvlRatio ?? null, // Anti-Rug juicy-pool cap; null = no cap
    ...
    maxVolatility:     u.maxVolatility     ?? 10,   // hard-reject candidates above this volatility (trade history: vol 8-10 healthy, disaster starts >10)
    maxVolumeMcapRatio: u.maxVolumeMcapRatio ?? 0.15, // pump-chase guard: reject when window volume > this fraction of mcap
    maxMcapVsTrendRatio: u.maxMcapVsTrendRatio ?? 1.5, // local-top guard: reject when mcap > this × pool's 4h median mcap (pool-memory snapshots)
    ...
    gateFailStickyMinutes: u.gateFailStickyMinutes ?? 30, // holder-gate failures stay rejected this long (anti-jitter, no added strictness)
  ...
  management: {
    ...
    limitExitOffsetPct:      u.limitExitOffsetPct      ?? 0.3,
    limitExitMaxWaitMinutes: u.limitExitMaxWaitMinutes ?? 30,
    limitExitMinUsd:         u.limitExitMinUsd         ?? 5,
    stopLossPct:           u.stopLossPct           ?? u.emergencyPriceDropPct ?? -50,
    stopLossCooldownHours: u.stopLossCooldownHours ?? 24,   // pool+token cooldown after a stop-loss close (blocks nuked-token re-entry)
    ...
    // Age-based deploy sizing — young tokens deploy at reduced size (a size scaler, NOT a hard filter)
    youngTokenAgeHours:    u.youngTokenAgeHours    ?? 6,    // tokens younger than this get the reduced size
    youngTokenSizeFactor:  u.youngTokenSizeFactor  ?? 0.5,  // deploy amount multiplier for young tokens
    // Trailing take-profit
    trailingTakeProfit:    u.trailingTakeProfit    ?? true,
    trailingTriggerPct:    u.trailingTriggerPct    ?? 3,    // activate trailing at X% PnL
    trailingDropPct:       u.trailingDropPct       ?? 1.5,  // close when drops X% from peak
    // Depth Guard — in-range dump guard (see depth-guard.js for the layer spec)
    depthGuardEnabled:      u.depthGuardEnabled      ?? false,
    depthAlertPct:          u.depthAlertPct          ?? 50,  // depth% that arms an episode
    depthRecoveryPct:       u.depthRecoveryPct       ?? 40,  // depth% that stands an episode down
    depthHardCutPct:        u.depthHardCutPct        ?? 65,  // kill-switch depth%
    depthGraceWindowMin:    u.depthGraceWindowMin    ?? 5,   // candle-trial window
    depthCandleVolRatio:    u.depthCandleVolRatio    ?? 0.8, // 1m vol must be >= this × prev candle
    depthBelowRangeBins:    u.depthBelowRangeBins    ?? 10,  // rug-gap backstop below range floor
    depthReentryCooldownMin: u.depthReentryCooldownMin ?? 45,
  ...
  jupiter: {
    ...
    // Referral REMOVED (user decision 2026-07-05): no third-party fees, ever.
    // Also hard-blocked in tools/wallet.js getJupiterReferralParams().
    referralAccount: "",
    referralFeeBps: 0,
  },
...
// ─── Feature master switches: Anti Rug & Anti Slip (2026-07-17) ───
// antiRugEnabled  = master switch for the Rug Detection entry-filter package.
//   NOTE: this agent (A2) is the un-armored CONTROL in the live duel — its
//   config already sits at baseline, so the switch is a no-op until Rug
//   Detection values are ever ported here. Kept for parity with Agent 1.
// antiSlipEnabled = master switch for Slippage Armor (limit exit + split swap).
//   OFF = plain market sells, no swap splitting (pre-2026-07-14 behavior).
// Both default ON. Read once at startup — toggling requires a process restart.
config.features = {
  antiRugEnabled:  u.antiRugEnabled  ?? true,
  antiSlipEnabled: u.antiSlipEnabled ?? true,
};
if (!config.features.antiRugEnabled) {
  config.screening.minTokenAgeHours     = null;    // no minimum age
  config.screening.minMcap              = 150_000; // baseline
  config.screening.minHolders           = 500;     // baseline default
  config.screening.maxVolatility        = 10;      // RUSH-guard baseline
  config.screening.maxFeeActiveTvlRatio = null;    // no juicy-pool cap
  config.screening.maxVolumeMcapRatio   = 0.15;    // RUSH-guard baseline
}
if (!config.features.antiSlipEnabled) {
  config.management.limitExitEnabled      = false;
  config.management.maxSwapPriceImpactPct = 999;   // never split
}
...
export function reloadScreeningThresholds() {
  try {
    ...
    if (fresh.maxVolumeMcapRatio != null) s.maxVolumeMcapRatio = fresh.maxVolumeMcapRatio;
    if (fresh.maxMcapVsTrendRatio != null) s.maxMcapVsTrendRatio = fresh.maxMcapVsTrendRatio;
    if (fresh.gateFailStickyMinutes != null) s.gateFailStickyMinutes = fresh.gateFailStickyMinutes;
