...
export function recordPoolDeploy(poolAddress, deployData) {
  ...
  // Stop-loss close → instant pool + token cooldown. A stop-loss means the token
  // nuked under us — re-entering within minutes is how the same token gets bought
  // twice on the way down. This is the single choke point for ALL close paths
  // (LLM close, deterministic close, /close command) via recordPerformance.
  if (String(deploy.close_reason || "").toLowerCase().includes("stop loss")) {
    const stopLossCooldownHours = Math.max(0, Number(config.management.stopLossCooldownHours ?? 24));
    if (stopLossCooldownHours > 0) {
      const reason = "stop loss close";
      const poolCooldownUntil = setPoolCooldown(entry, stopLossCooldownHours, reason);
      const mintCooldownUntil = setBaseMintCooldown(db, entry.base_mint, stopLossCooldownHours, reason);
...
/**
 * Apply a cooldown from outside the recordPoolDeploy flow (e.g. depth-guard
 * cuts, which fire from the fast poller before performance is recorded).
 */
export function applyExternalCooldown({ pool_address, base_mint = null, hours, reason }) {
  if (!pool_address || !(hours > 0)) return { applied: false };
  const db = load();
  let entry = db[pool_address];
  if (!entry) {
    entry = db[pool_address] = {
  ...
  const mintCooldownUntil = setBaseMintCooldown(db, base_mint || entry.base_mint, hours, reason);
  save(db);
  log("pool-memory", `External cooldown for ${entry.name} until ${poolCooldownUntil} (${reason})${mintCooldownUntil ? ` + mint until ${mintCooldownUntil}` : ""}`);
  return { applied: true, poolCooldownUntil, mintCooldownUntil };
}
...
// ─── Holder-gate failure stickiness ────────────────────────────
// When a mint fails a holder-quality gate (top10 / bot holders), persist the
// failure so a jittery re-fetch minutes later can't flip it back to passing.
// Stored as a top-level `_gate_failures` map (underscore = meta, not a pool
// entry; other consumers iterate entries via `base_mint` and skip it).
const GATE_FAILURE_KEY = "_gate_failures";
const GATE_FAILURE_MAX_AGE_MS = 24 * 60 * 60 * 1000; // prune entries older than 24h

export function recordGateFailure(baseMint, reason) {
  if (!baseMint) return;
  const db = load();
  const failures = db[GATE_FAILURE_KEY] && typeof db[GATE_FAILURE_KEY] === "object" && !Array.isArray(db[GATE_FAILURE_KEY])
    ? db[GATE_FAILURE_KEY]
    : {};
  const now = Date.now();
  for (const [mint, entry] of Object.entries(failures)) {
    if (!entry?.failed_at || now - new Date(entry.failed_at).getTime() > GATE_FAILURE_MAX_AGE_MS) {
      delete failures[mint];
    }
  }
  failures[baseMint] = {
    reason: sanitizeStoredNote(reason) || "holder gate",
    failed_at: new Date(now).toISOString(),
  };
  db[GATE_FAILURE_KEY] = failures;
  save(db);
  log("pool-memory", `Gate failure recorded for mint ${baseMint.slice(0, 8)}: ${reason}`);
}

/**
 * Returns { reason, minutes_ago } if the mint failed a holder gate within the
 * last `stickyMinutes`, else null. Anti-jitter: the recorded failure wins over
 * a fresh (possibly noisy) re-fetch for the duration of the sticky window.
 */
export function getStickyGateFailure(baseMint, stickyMinutes = 30) {
  if (!baseMint) return null;
  const minutes = Number(stickyMinutes);
  if (!Number.isFinite(minutes) || minutes <= 0) return null;
  const db = load();
  const entry = db[GATE_FAILURE_KEY]?.[baseMint];
  if (!entry?.failed_at) return null;
  const ageMs = Date.now() - new Date(entry.failed_at).getTime();
  if (!Number.isFinite(ageMs) || ageMs < 0 || ageMs > minutes * 60 * 1000) return null;
  return { reason: entry.reason || "holder gate", minutes_ago: Math.floor(ageMs / 60000) };
}

// ─── Skip-notification dedup ───────────────────────────────────
// A hot token that survives gates but gets skipped at the conviction stage
// re-appears every cycle — the ⏭️ Skip Telegram notification fires once per
// SKIP_NOTIFY_DEDUP_HOURS per mint. Same meta-map pattern as _gate_failures.
const SKIP_NOTIFY_KEY = "_skip_notifications";
const SKIP_NOTIFY_DEDUP_HOURS = 6;
const SKIP_NOTIFY_MAX_AGE_MS = 24 * 60 * 60 * 1000;

/**
 * Check-and-record: returns true (and records now) if no skip notification
 * was sent for this mint in the last dedup window, else false.
 */
export function shouldNotifySkip(baseMint, dedupHours = SKIP_NOTIFY_DEDUP_HOURS) {
  if (!baseMint) return false;
  const db = load();
  const map = db[SKIP_NOTIFY_KEY] && typeof db[SKIP_NOTIFY_KEY] === "object" && !Array.isArray(db[SKIP_NOTIFY_KEY])
    ? db[SKIP_NOTIFY_KEY]
    : {};
  const now = Date.now();
  const last = map[baseMint] ? new Date(map[baseMint]).getTime() : null;
  if (last != null && Number.isFinite(last) && now - last < dedupHours * 3600 * 1000) return false;
  for (const [mint, at] of Object.entries(map)) {
    if (!at || now - new Date(at).getTime() > SKIP_NOTIFY_MAX_AGE_MS) delete map[mint];
  }
  map[baseMint] = new Date(now).toISOString();
  db[SKIP_NOTIFY_KEY] = map;
  save(db);
  return true;
}

/**
 * Median mcap of a pool's snapshots within the last `hours` (default 4h).
 * Used by the screening "buying local top" gate: an entry mcap far above the
 * recent median means we'd be deploying into a pump. Returns null when fewer
 * than `minSamples` mcap-bearing snapshots exist — snapshots only exist for
 * pools we've held positions in, so absence of data must never reject.
 */
export function getSnapshotMedianMcap(poolAddress, { hours = 4, minSamples = 5 } = {}) {
  if (!poolAddress) return null;
  const db = load();
  const snaps = db[poolAddress]?.snapshots;
  if (!Array.isArray(snaps) || snaps.length === 0) return null;
  const cutoff = Date.now() - hours * 60 * 60 * 1000;
  const mcaps = snaps
    .filter((s) => s?.mcap != null && Number.isFinite(Number(s.mcap)) && Number(s.mcap) > 0 && new Date(s.ts).getTime() >= cutoff)
    .map((s) => Number(s.mcap))
    .sort((a, b) => a - b);
  if (mcaps.length < minSamples) return null;
  const mid = Math.floor(mcaps.length / 2);
  return mcaps.length % 2 ? mcaps[mid] : (mcaps[mid - 1] + mcaps[mid]) / 2;
}
