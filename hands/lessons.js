...
import { recordClose } from "./telemetry.js";
...
export async function recordPerformance(perf) {
  ...
  // Gross = pure LP price PnL + fees earned, ignoring network cost. Guards
  // below check the gross number since they're sanity checks on Meteora's
  // reported values, not on our own gas accounting.
  const pnl_usd_gross = (perf.final_value_usd + perf.fees_earned_usd) - perf.initial_value_usd;
  const pnl_pct_gross = perf.initial_value_usd > 0
    ? (pnl_usd_gross / perf.initial_value_usd) * 100
    : 0;
  ...
  const suspiciousAbsurdClosedPnl =
    Number.isFinite(pnl_pct_gross) &&
    perf.initial_value_usd >= 20 &&
    pnl_pct_gross <= -90 &&
    !closeReasonText.includes("stop loss");

  if (suspiciousAbsurdClosedPnl) {
    log("lessons_warn", `Skipped absurd closed PnL record for ${perf.pool_name || perf.pool}: pnl_pct=${pnl_pct_gross.toFixed(2)} reason=${perf.close_reason}`);
    return;
  }

  // Net out actual network fees (deploy + claim + close transactions,
  // captured by tools/dlmm.js via getTxFeesSol) so pnl_usd reflects real
  // take-home, not just Meteora's LP-position-only accounting. Swap-back
  // slippage/gas is a known remaining gap — see project memory
  // meridian-gas-net-pnl-pending.
  const gasSol = Number(perf.gas_sol) || 0;
  const solPriceUsd = Number.isFinite(perf.sol_price_usd) && perf.sol_price_usd > 0 ? perf.sol_price_usd : null;
  const gasUsd = solPriceUsd != null ? gasSol * solPriceUsd : 0;
  const pnl_usd = pnl_usd_gross - gasUsd;
  ...
  const entry = {
    ...
    pnl_usd_gross: Math.round(pnl_usd_gross * 100) / 100,
    gas_sol: Math.round(gasSol * 1e6) / 1e6,
  ...
  recordClose(entry); // telemetry — passive, never throws
...
/**
 * Attach swap-back (post-close Jupiter autoswap) slippage data to the
 * performance entry it belongs to. Observational only — pnl_usd stays
 * untouched so A/B metrics remain comparable across the tracking rollout;
 * scaling analysis reads entry.swap_back directly.
 */
export function recordSwapBack(positionAddress, swapBack) {
  if (!positionAddress || !swapBack) return false;
  const data = load();
  for (let i = data.performance.length - 1; i >= 0; i--) {
    if (data.performance[i].position === positionAddress) {
      data.performance[i].swap_back = swapBack;
      save(data);
      return true;
    }
  }
  log("lessons_warn", `swap_back: no performance entry for position ${positionAddress}`);
  return false;
}
...
export function evolveThresholds(perfData, config) {
  // ERA MIRROR (08-20): evolusi config DIMATIKAN — screening dipegang paper engine +
  // brain-wb-distiller, bukan cli. Insiden: evolver naikin minFeeActiveTvlRatio 0→0.05
  // pas close MANLET 08-18 → SEMUA deploy mirror keblok SAFETY_BLOCK 2 hari kemudian.
  // recordPerformance tetep jalan (data learning kekumpul), cuma mutasi config yang dicabut.
  return null;
  ...
  // ── 2. maxVolatility ────────────────────────────────────────────
  // Lower the ceiling if high-volatility pools consistently underperform.
  {
    ...
    if (loserVols.length >= 2 && winnerVols.length >= 1 && isFiniteNum(current)) {
      const avgLoserVol  = avg(loserVols);
      const avgWinnerVol = avg(winnerVols);
      // Only tighten if there's a clear gap (losers consistently more volatile)
      if (avgLoserVol - avgWinnerVol >= 2) {
        // Set ceiling just above worst winner — losers above it get filtered next time
        const maxWinnerVol = Math.max(...winnerVols);
        const target = Math.min(maxWinnerVol * 1.15, current);
        const newVal = clamp(Number(nudge(current, target, MAX_CHANGE_PER_STEP).toFixed(2)), 3.0, 10.0);
        if (newVal < current) {
          changes.maxVolatility = newVal;
          rationale.maxVolatility = `Loser avg volatility ${avgLoserVol.toFixed(2)} vs winner avg ${avgWinnerVol.toFixed(2)} — lowered ceiling from ${current} → ${newVal}`;
        }
      }
    }
  }
