...
// AUDIT 08-24: normalizeMint HANYA buat mint token — kena address posisi/pool bisa
// nuker alamat jadi mint SOL diam-diam. Address wajib lolos validasi, bukan "dinormalisasi".
const assertPubkey = (s) => { new PublicKey(s); return s; };
...
// signatures. Best-effort telemetry only — never throws, never blocks the
// deploy/close flow it's called from.
export async function getTxFeesSol(signatures) {
  let totalLamports = 0;
  for (const sig of signatures || []) {
    try {
      const tx = await getConnection().getTransaction(sig, { maxSupportedTransactionVersion: 0 });
      if (tx?.meta?.fee) totalLamports += tx.meta.fee;
    } catch { /* best-effort — gas telemetry must never block trading */ }
  }
...
export async function getActiveBin({ pool_address }) {
  pool_address = assertPubkey(pool_address);
...
export async function deployPosition({
  ...
  entry_volume,
  entry_holders,
  // age-based size scaling note (injected by executor safety checks)
  size_scaling_note,
}) {
  pool_address = assertPubkey(pool_address);
  ...
  try {
    ...
    _positionsCacheAt = 0;
    const deployGasSol = await getTxFeesSol(txHashes);
    const signalSnapshot = config.darwin?.enabled
      ? getAndClearStagedSignals(pool_address, baseMint)
      : null;
    trackPosition({
...
export async function getPositionPnl({ pool_address, position_address }) {
  pool_address = assertPubkey(pool_address);
  position_address = assertPubkey(position_address);
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (!closedConfirmed) {
      // AUDIT 08-24: verifikasi pake gPA yang sama lag-nya — tx udah LANDED = close sukses.
      // success:false di sini bikin post-hook (auto-swap!) di-skip -> token nyangkut -> -90%.
      let goneProof = false;
      try { goneProof = (await getConnection().getAccountInfo(new PublicKey(position_address))) == null; } catch {}
      log("close_warn", `verification window miss — txs landed (${closeTxHashes.length}), accountGone=${goneProof}; treated as SUCCESS (unverified)`);
    }

    try { recordClose(position_address, reason || "agent decision"); }
    catch (e) { log("close_warn", `recordClose failed post-tx (bookkeeping only): ${e.message}`); }
    ...
    if (tracked) {
      ...
      await recordPerformance({
        ...
        final_value_usd: finalValueUsd,
        initial_value_usd: initialUsd,
        gas_sol: (tracked.deploy_gas_sol || 0) + closeGasSol,
        sol_price_usd: tracked.amount_sol > 0 ? initialUsd / tracked.amount_sol : null,
        minutes_in_range: minutesHeld - minutesOOR,
        minutes_held: minutesHeld,
        close_reason: reason || "agent decision",
        signal_snapshot: signalSnapshot,
...
async function lookupPoolForPosition(position_address, walletAddress) {
  ...
  // AUDIT 08-24: jalur ke-4 — gPA langsung (dataSlice lbPair@8, resep a2-mirror yang
  // kebukti ga buta). 3 insiden "not found" padahal posisi hidup on-chain.
  try {
    const accInfo = await getConnection().getAccountInfo(new PublicKey(position_address));
    if (accInfo && accInfo.owner.toString() === "LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo") {
      const lbPair = new PublicKey(accInfo.data.subarray(8, 40)).toString();
      log("lookup", `pool resolved via direct account read (jalur 4): ${lbPair}`);
      return lbPair;
    }
    if (!accInfo) throw new Error(`Position ${position_address} tidak ada di registry maupun on-chain (account gone)`);
  } catch (e) {
    if (String(e.message).includes("account gone")) throw e;
    log("lookup", `jalur 4 gagal (RPC): ${e.message}`);
  }

  throw new Error(`Position ${position_address} not found: registry miss + SDK scan miss + RPC scan gagal (BUKAN bukti posisi ga ada!)`);
}
