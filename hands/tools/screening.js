...
import { getSnapshotMedianMcap, getStickyGateFailure, isBaseMintOnCooldown, isPoolOnCooldown } from "../pool-memory.js";
import { recordCandidate } from "../telemetry.js";
...
function getRawPoolScreeningRejectReason(pool, s) {
  ...
  // Anti-Rug: juicy-pool trap — fee/TVL too HIGH = wash-traded bait (32% rug in 707-event study)
  if (s.maxFeeActiveTvlRatio != null && Number.isFinite(feeActiveTvlRatio) && feeActiveTvlRatio > s.maxFeeActiveTvlRatio) {
    return `fee/active-TVL ${feeActiveTvlRatio} above maxFeeActiveTvlRatio ${s.maxFeeActiveTvlRatio} (juicy-pool trap)`;
  }
...
export async function discoverPools({
  ...
  const thresholdedRawPools = rawPools.filter((pool) => {
    ...
    // Telemetry: raw-stage rejects, only on the 30-min screening cadence — the
    // 45s opportunity poll would flood the log with duplicate raw rejects.
    if (cycle === "screening") {
      try {
        recordCandidate({ cycle, pool: condensePool(pool), verdict: "reject", failed_gates: [reason] });
      } catch { /* telemetry never breaks a cycle */ }
    }
 ...
 */
export async function getTopCandidates({ limit = 10, cycle = "screening" } = {}) {
  ...
  const discovery = source === "gmgn"
    ...
    : await discoverPools({ page_size: 50, cycle });
  let { pools } = discovery;
  ...
  const maxVolatility = Number(config.screening.maxVolatility ?? 0);
  const maxVolumeMcapRatio = Number(config.screening.maxVolumeMcapRatio ?? 0);
  const maxMcapVsTrendRatio = Number(config.screening.maxMcapVsTrendRatio ?? 0);

  const eligible = pools
    .filter((p) => {
      // Collect ALL failing gates (not just the first) so telemetry can audit
      // every gate independently. Cheap parametric gates are always evaluated;
      // file-backed gates (local-top / cooldowns / sticky) keep the original
      // short-circuit and only run while the candidate is still clean.
      const failures = [];
      const tvl = Number(p.tvl ?? p.active_tvl ?? 0);
      if (Number.isFinite(minTvl) && minTvl > 0 && tvl < minTvl) {
        failures.push(`TVL $${tvl} below minTvl $${minTvl}`);
      }
      if (Number.isFinite(maxTvl) && maxTvl > 0 && tvl > maxTvl) {
        failures.push(`TVL $${tvl} above maxTvl $${maxTvl}`);
      }
      ...
      if (Number.isFinite(minFeeActiveTvlRatio) && minFeeActiveTvlRatio > 0 && (!Number.isFinite(feeActiveTvlRatio) || feeActiveTvlRatio < minFeeActiveTvlRatio)) {
        failures.push(`fee/active-TVL ${Number.isFinite(feeActiveTvlRatio) ? feeActiveTvlRatio : "unknown"} below minFeeActiveTvlRatio ${minFeeActiveTvlRatio}`);
      }
      if (maxFeeActiveTvlRatio != null && Number.isFinite(feeActiveTvlRatio) && feeActiveTvlRatio > maxFeeActiveTvlRatio) {
        failures.push(`fee/active-TVL ${feeActiveTvlRatio} above maxFeeActiveTvlRatio ${maxFeeActiveTvlRatio} (juicy-pool trap)`);
      }
      if (!isUsableVolatility(p.volatility)) {
        failures.push(`volatility ${p.volatility ?? "unknown"} unusable`);
      } else if (Number.isFinite(maxVolatility) && maxVolatility > 0 && Number(p.volatility) > maxVolatility) {
        failures.push(`volatility ${p.volatility} above maxVolatility ${maxVolatility}`);
      }
      // Pump-chase guard: window volume dwarfing mcap = violent pump in progress
      const volumeWindow = Number(p.volume_window);
      const mcap = Number(p.mcap);
      if (
        Number.isFinite(maxVolumeMcapRatio) && maxVolumeMcapRatio > 0 &&
        Number.isFinite(volumeWindow) && Number.isFinite(mcap) && mcap > 0 &&
        volumeWindow / mcap > maxVolumeMcapRatio
      ) {
        failures.push(`pump-chase guard: volume ${((volumeWindow / mcap) * 100).toFixed(1)}% of mcap (max ${(maxVolumeMcapRatio * 100).toFixed(0)}%)`);
      }
      // Local-top guard: mcap far above the pool's recent median (pool-memory snapshots).
      // Best-effort — snapshots only exist for pools we've held, missing data never rejects.
      if (failures.length === 0 && Number.isFinite(maxMcapVsTrendRatio) && maxMcapVsTrendRatio > 0 && Number.isFinite(mcap) && mcap > 0) {
        const medianMcap = getSnapshotMedianMcap(p.pool);
        if (medianMcap != null && mcap > maxMcapVsTrendRatio * medianMcap) {
          failures.push(`buying local top: mcap $${Math.round(mcap)} vs 4h median $${Math.round(medianMcap)} (max ${maxMcapVsTrendRatio}x)`);
        }
      }
      if (occupiedPools.has(p.pool)) {
        failures.push("already have an open position in this pool");
      }
      if (occupiedMints.has(p.base?.mint)) {
        failures.push("already holding this base token in another pool");
      }
      if (failures.length === 0 && isPoolOnCooldown(p.pool)) {
        log("screening", `Filtered cooldown pool ${p.name} (${p.pool.slice(0, 8)})`);
        failures.push("pool cooldown active");
      }
      if (failures.length === 0 && isBaseMintOnCooldown(p.base?.mint)) {
        log("screening", `Filtered cooldown token ${p.base?.symbol} (${p.base?.mint?.slice(0, 8)})`);
        failures.push("token cooldown active");
      }
      // Anti-jitter: a mint that failed a holder gate (top10 / bot holders) stays
      // rejected for gateFailStickyMinutes even if a fresh fetch would now pass.
      if (failures.length === 0) {
        const stickyGate = getStickyGateFailure(p.base?.mint, config.screening.gateFailStickyMinutes ?? 30);
        if (stickyGate) {
          log("screening", `Filtered sticky gate-failure token ${p.base?.symbol} (${p.base?.mint?.slice(0, 8)}) — ${stickyGate.reason} ${stickyGate.minutes_ago}m ago`);
          failures.push(`failed ${stickyGate.reason} ${stickyGate.minutes_ago}m ago — sticky rejection`);
        }
      }
      if (failures.length > 0) {
        pushFilteredReason(filteredOut, p, failures[0]);
        recordCandidate({ cycle, pool: p, verdict: "reject", failed_gates: failures, degen: degenScore(p, config.opportunity) });
        return false;
      }
      return true;
    })
  ...
  if (config.screening.avoidPvpSymbols && eligible.length > 0) {
    ...
    if (config.screening.blockPvpSymbols) {
      ...
      pvpRemoved.forEach((p) => {
        ...
        recordCandidate({ cycle, pool: p, verdict: "reject", failed_gates: ["PVP hard filter"], degen: degenScore(p, config.opportunity) });
      });
  ...
  if (eligible.length > 0) {
    ...
    const filtered = eligible.filter((p) => {
      if (p.dev && isDevBlocked(p.dev)) {
        log("dev_blocklist", `Filtered blocked deployer ${p.dev.slice(0, 8)} token ${p.base?.symbol}`);
        pushFilteredReason(filteredOut, p, "blocked deployer");
        recordCandidate({ cycle, pool: p, verdict: "reject", failed_gates: ["blocked deployer"] });
        return false;
      }
      return true;
    });
  ...
  if (config.indicators.enabled && eligible.length > 0) {
    ...
    const confirmedEligible = eligible.filter((pool) => {
      ...
      recordCandidate({ cycle, pool, verdict: "reject", failed_gates: [`indicator reject: ${confirmation.reason}`] });
  ...
  // Telemetry: record the final shortlist as passes (rejects were recorded at
  // each gate above). Candidates cut only by the score sort/limit get no event.
  for (const p of eligible) {
    recordCandidate({ cycle, pool: p, verdict: "pass", degen: degenScore(p, config.opportunity) });
  }
