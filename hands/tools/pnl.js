...
export async function computePositions(walletAddress) {
  ...
  if (flat.length === 0) {
    // AUDIT 08-24: kosong-krn-RPC-lag != kosong-beneran. Kalau registry masih pegang
    // posisi open, JANGAN klaim "0 posisi" — lempar biar caller pake fallback.
    try {
      const { getTrackedPositions } = await import("../state.js");
      if (getTrackedPositions(true).length > 0)
        throw new Error("gPA balikin 0 posisi tapi registry punya open — RPC ga dipercaya");
    } catch (e) { if (String(e.message).includes("ga dipercaya")) throw e; }
