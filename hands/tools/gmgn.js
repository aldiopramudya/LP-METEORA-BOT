...
/**
 * Single-token wallet-composition analysis (ported from Agent 1's gmgn.js) —
 * used by the passive post-deploy audition (telemetry entry_gmgn events).
 * One paced /v1/token/info call; returns { available: false } gracefully on
 * missing key / no data / error so callers can always emit coverage info.
 */
export async function getGmgnTokenAnalysis({ mint }) {
  if (!mint || !hasGmgnApiKey()) {
    return { mint, available: false, reason: "GMGN API key not configured" };
  }
  try {
    ...
    if (!info || typeof info !== "object") return { mint, available: false, reason: "No data" };
    ...
    return {
      mint,
      available: true,
      smart_wallets: optionalNum(tags.smart_wallets) ?? 0,
      renowned_wallets: optionalNum(tags.renowned_wallets) ?? 0,
      sniper_wallets: optionalNum(tags.sniper_wallets) ?? 0,
      rat_trader_wallets: optionalNum(tags.rat_trader_wallets) ?? 0,
      bundler_wallets: optionalNum(tags.bundler_wallets) ?? 0,
      top_wallets: optionalNum(tags.top_wallets) ?? 0,
      bot_degen_rate: optionalNum(stat.bot_degen_rate),
      rat_trader_pct: optionalNum(stat.top_rat_trader_percentage) ?? 0,
      bundler_trader_pct: optionalNum(stat.top_bundler_trader_percentage) ?? 0,
      creator_status: dev.creator_token_status || null,
      top_10_holder_rate: optionalNum(dev.top_10_holder_rate),
      locked_ratio: optionalNum(info.locked_ratio),
    };
  } catch (error) {
    log("gmgn", `token analysis failed for ${String(mint).slice(0, 8)}: ${error.message}`);
    return { mint, available: false, reason: error.message };
  }
}
