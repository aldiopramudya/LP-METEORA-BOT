...
import { getLastBriefingDate, setLastBriefingDate, getTrackedPosition, getTrackedPositions, setPositionInstruction, updatePnlAndCheckExits, confirmPeak, registerExitSignal, markOorNotified } from "./state.js";
import { getActiveStrategy } from "./strategy-library.js";
import { recordPositionSnapshot, recallForPool, addPoolNote, recordGateFailure, shouldNotifySkip } from "./pool-memory.js";
import { recordCandidate, recordTick } from "./telemetry.js";
...
import { evaluateDepthGuard, onDepthCutClosed } from "./depth-guard.js";
...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    if (positions.length === 0) {
      ...
      runScreeningCycle({ silent: true }).catch((e) => log("cron_error", `Triggered screening failed: ${e.message}`));
      return mgmtReport;
    }

    // Snapshot + load pool memory. Best-effort current mcap per pool so snapshots
    // build the 4h mcap trend used by the screening local-top gate (getSnapshotMedianMcap).
    // The same fetched detail feeds the telemetry tick — no extra API calls.
    const detailByPool = new Map();
    await Promise.all([...new Set(positions.map((p) => p.pool))].map(async (pool) => {
      const detail = await getPoolDetail({ pool_address: pool, timeframe: config.screening.timeframe }).catch(() => null);
      if (detail) detailByPool.set(pool, detail);
    }));
    const positionData = positions.map((p) => {
      const detail = detailByPool.get(p.pool) ?? null;
      const mcap = Number(detail?.token_x?.market_cap);
      recordPositionSnapshot(p.pool, { ...p, mcap: Number.isFinite(mcap) && mcap > 0 ? mcap : null });
      recordTick({ pos: p, detail });
    ...
    // OOR notify — ONCE per OOR episode (oor_notified flag, cleared when the
    // position comes back in range). Fires in silent cron cycles too: event
    // notifications are not cycle spam. Direction from active bin vs range.
    if (telegramEnabled()) {
      for (const p of positions) {
        if (p.in_range || (p.minutes_out_of_range ?? 0) < config.management.outOfRangeWaitMinutes) continue;
        const tracked = getTrackedPosition(p.position);
        if (tracked?.oor_notified) continue;
        const direction = p.active_bin != null && p.upper_bin != null && p.active_bin > p.upper_bin
          ? "above"
          : p.active_bin != null && p.lower_bin != null && p.active_bin < p.lower_bin
            ? "below"
            : null;
        markOorNotified(p.position);
        notifyOutOfRange({
          pair: p.pair,
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    const passing = allCandidates.filter(({ pool, ti }) => {
      ...
      // Holder-quality hard gates on EVERY candidate (not just the lone-candidate path).
      // Same audit fields + gmgn fallbacks as getLoneCandidateSkipReason. Missing data
      // never rejects; a failure is persisted (recordGateFailure) so a jittery re-fetch
      // minutes later can't flip the mint back to passing (sticky rejection in screening).
      const top10Pct = Number(ti?.audit?.top_holders_pct ?? pool.gmgn_token_info_top10_pct ?? pool.gmgn_top10_holder_pct);
      const botPct = Number(ti?.audit?.bot_holders_pct ?? pool.gmgn_bot_degen_pct);
      ...
      if (Number.isFinite(top10Pct) && maxTop10Pct != null && top10Pct > maxTop10Pct) {
        const reason = `top10 gate (${top10Pct}% above ${maxTop10Pct}%)`;
        log("screening", `Top10-holder filter: dropped ${pool.name} — ${reason}`);
        recordGateFailure(pool.base?.mint, reason);
        recordCandidate({ cycle: "screening", pool, verdict: "reject", failed_gates: [reason], extra: { top10_pct: top10Pct, bot_pct: Number.isFinite(botPct) ? botPct : null } });
        filteredOut.push({ name: pool.name, reason });
        return false;
      }
      if (Number.isFinite(botPct) && maxBotHoldersPct != null && botPct > maxBotHoldersPct) {
        const reason = `bot-holder gate (${botPct}% above ${maxBotHoldersPct}%)`;
        log("screening", `Bot-holder filter: dropped ${pool.name} — ${reason}`);
        recordGateFailure(pool.base?.mint, reason);
        recordCandidate({ cycle: "screening", pool, verdict: "reject", failed_gates: [reason], extra: { top10_pct: Number.isFinite(top10Pct) ? top10Pct : null, bot_pct: botPct } });
        filteredOut.push({ name: pool.name, reason });
        return false;
      }
      return true;
    ...
    if (passing.length === 1) {
      ...
      if (skipReason) {
        ...
        // ⏭️ Skip notification — finalist passed hard gates, lost at conviction.
        // Deduped 6h per mint so a hot token skipped every cycle notifies once.
        const skipMint = passing[0].pool?.base?.mint || `name:${candidateName}`;
        if (shouldNotifySkip(skipMint)) {
          notifySkip({ pair: candidateName, reason: skipReason.slice(0, 220) }).catch(() => {});
        }
        return screenReport;
      }
    }
    ...
    const candidateBlocks = passing.map(({ pool, sw, n, ti, mem }, i) => {
      ...
      if (config.darwin?.enabled) {
        ...
        stageSignals(pool.pool, {
          ...
          bot_pct:               ti?.audit?.bot_holders_pct != null ? Number(ti.audit.bot_holders_pct) : null,
          degen:                 Number(degenScore(pool, config.opportunity).toFixed(1)),
        });
      }
...
- Keep the whole report compact and highly scannable for Telegram.
    ...
    if (/⛔\s*NO DEPLOY/i.test(content)) {
      ...
      // ⏭️ Skip notification for the LLM's named best candidate (finalists only,
      // never raw gate rejects). Deduped 6h per mint via pool-memory.
      try {
        const cleaned = stripThink(content);
        const bestName = cleaned.match(/BEST LOOKING CANDIDATE\s*\n\s*(.+)/i)?.[1]?.trim();
        if (bestName && !/^(none|n\/a|-)$/i.test(bestName) && !/^<.*>$/.test(bestName)) {
          const match = passing.find((c) => (c.pool?.name || "").toLowerCase() === bestName.toLowerCase());
          const skipMint = match?.pool?.base?.mint || `name:${bestName}`;
          const why = cleaned.match(/WHY SKIPPED\s*\n([\s\S]{0,400}?)(\n\s*\n|$)/i)?.[1];
          const reason = (why || "no valid entry this cycle").replace(/\s+/g, " ").trim().slice(0, 220);
          if (shouldNotifySkip(skipMint)) {
            notifySkip({ pair: bestName, reason }).catch(() => {});
          }
        }
...
export function startCronJobs() {
  ...
  // Cron-invoked cycles run SILENT — no live message / cycle-report spam.
  // Event notifications (deploy/close/OOR/skip) still fire standalone, and
  // user-invoked paths (/screen, Telegram chat) keep their live messages.
  ...
  const screenTask = cron.schedule(`*/${Math.max(1, config.schedule.screeningIntervalMin)} * * * *`, () => runScreeningCycle({ silent: true }));
  ...
  const pnlPollInterval = setInterval(async () => {
    ...
    try {
      ...
      for (const p of result.positions) {
        ...
        // Detect an exit signal this tick (rule-based exits, then deterministic close
        // rules, then the Depth Guard — SL/trailing/rules always win over the guard).
        const exit = updatePnlAndCheckExits(p.position, p, config.management);
        ...
        if (exit) { signal = exit.action; reason = exit.reason; }
          ...
          const guardCut = await evaluateDepthGuard(p).catch((e) => {
            log("cron_error", `Depth guard evaluation failed: ${e.message}`);
            return null;
          });
          if (guardCut) { signal = "DEPTH_GUARD"; reason = guardCut.reason; rule = "DEPTH_GUARD"; }
        }
        ...
        try {
          ...
          if (rule === "DEPTH_GUARD") onDepthCutClosed(p);
        } catch (e) {
  ...
  if (config.opportunity.enabled) {
    ...
    opportunityPollInterval = setInterval(async () => {
      ...
      try {
        ...
        const top = await getTopCandidates({ limit: config.opportunity.limit, cycle: "opportunity" }).catch(() => null);
        ...
        // the 45s poll cheap.
        let trigger = null;
        const oppMinAgeHours = Number(config.management.youngTokenAgeHours ?? 6);
        for (const c of candidates) {
          // Young tokens don't get the fast path — they can still deploy through the
          // regular screening cycle (at reduced young-token sizing). Unknown age = allow.
          if (c.token_age_hours != null && Number.isFinite(oppMinAgeHours) && oppMinAgeHours > 0 && c.token_age_hours < oppMinAgeHours) {
            log("cron", `[Opportunity] Skipping ${c.name} — token age ${c.token_age_hours}h < ${oppMinAgeHours}h (young-token floor)`);
            continue;
          }
...
function getLoneCandidateSkipReason({ pool, sw, n, ti } = {}) {
  ...
  if (Number.isFinite(top10Pct) && top10Pct > config.screening.maxTop10Pct) {
    recordGateFailure(pool.base?.mint, `top10 gate (${top10Pct}% above ${config.screening.maxTop10Pct}%)`);
  ...
  if (Number.isFinite(botPct) && botPct > config.screening.maxBotHoldersPct) {
    recordGateFailure(pool.base?.mint, `bot-holder gate (${botPct}% above ${config.screening.maxBotHoldersPct}%)`);
...
Focus on: hold duration, entry/exit timing, what win rates look like, whether scalpers or holders dominate.`,
  ...
  (async () => {
    try {
      await runScreeningCycle({ silent: true }); // startup kick — quiet (was the last loud leak)
    } catch (e) {
