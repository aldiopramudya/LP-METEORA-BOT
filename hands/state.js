...
function load() {
  ...
  try {
    ...
    // AUDIT 08-24: fail-open ke {} = nuke-all registry (save berikutnya nulis kosong,
    // 402 posisi lenyap, semua close jadi "not found"). Corrupt = STOP, jangan reset.
    log("state_error", `state.json CORRUPT — refuse to operate: ${err.message}`);
    throw new Error(`state.json corrupt — restore dari backup, JANGAN direset: ${err.message}`);
  }
}
...
function save(state) {
  try {
    ...
    // AUDIT 08-24: atomic write — 3 penulis paralel (agent/cli/mirror), non-atomic = nuke-all
...
export function markInRange(position_address) {
  ...
  const pos = state.positions[position_address];
  if (!pos) return;
  if (pos.out_of_range_since || pos.oor_notified) {
    pos.out_of_range_since = null;
    pos.oor_notified = false; // new OOR episode may notify again
    save(state);
...
/**
 * Mark that the once-per-episode OOR Telegram notification was sent for this
 * position. Cleared when the position comes back in range so a NEW OOR
 * episode can notify again.
 */
export function markOorNotified(position_address) {
  const state = load();
  const pos = state.positions[position_address];
  if (!pos || pos.oor_notified) return;
  pos.oor_notified = true;
  save(state);
}
...
export function syncOpenPositions(active_addresses) {
  // AUDIT 08-24 (AKAR insiden Dinger/MADE2): gPA kosong krn RPC lag pernah dianggep
  // "ga punya posisi" -> SEMUA posisi di-closed permanen -> SL/trailing/OOR MATI.
  // 376/402 entry registry ke-close palsu lewat jalur ini. Empty list = no-op, titik.
  if (!Array.isArray(active_addresses) || active_addresses.length === 0) {
    const openCount = Object.values(load().positions).filter((p) => !p.closed).length;
    if (openCount > 0) log("state", `sync: on-chain list KOSONG tapi registry punya ${openCount} open — RPC ga dipercaya, skip sync`);
    return;
  }
  const state = load();
