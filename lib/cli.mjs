...
export class Cli {
  ...
  /**
   * Reclaim rent from empty token accounts (F1). Dry-run unless `live` is explicitly
   * true — a live sweep must always be an explicit, deliberate call.
   */
  cleanupEmptyAtas({ live = false, maxPerTx = null } = {}) {
    return this.run(
      ["cleanup-empty-atas", ...(live ? ["--live"] : ["--dry-run"]), ...(maxPerTx ? ["--max-per-tx", String(maxPerTx)] : [])],
      240000,
    );
  }
